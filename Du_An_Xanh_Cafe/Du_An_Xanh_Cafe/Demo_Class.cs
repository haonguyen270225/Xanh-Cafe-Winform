...
namespace Du_An_Xanh_Cafe
{
    public partial class Demo_Class : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            bool Check = Xu_Ly_Chuoi.CompareString(tmp1, tmp2);

        }

        private void button2_Click(object sender, EventArgs e)
        {
            txt1.Text = Xu_Ly_Chuoi.DeleteSpase(txt1.Text);
            txt2.Text = Xu_Ly_Chuoi.DeleteSpase(txt2.Text);
            MessageBox.Show("Xoa khoan trang dau va cuoi chuoi !", "Thong bao !");
        }

        private void button3_Click(object sender, EventArgs e)
        {
            if(Xu_Ly_Chuoi.CheckUserName(txt1.Text) == false)
            {
                MessageBox.Show("Ten username khong hop le !");
            }
            else if(Xu_Ly_Chuoi.CheckUserName(txt2.Text) == false)
            {
                MessageBox.Show("Ten username khong hop le !");
            }
            else
